...
class FailureDetectorPlugin(
    ...
    octoprint.plugin.SimpleApiPlugin,  # <-- NEW: For handling API calls
    octoprint.plugin.AssetPlugin      # <-- NEW: For serving JS/CSS files
):

    # ... (keep the __init__, on_after_startup, and load_model methods exactly as they are)
    def __init__(self):
    ...
    # ~~ SettingsPlugin mixin (No changes needed) ~~
    ...
    def on_api_command(self, command, data):
        if command == "force_check":
            ...
            # Run the check in a background thread to not block the API response
            check_thread = threading.Thread(target=self.perform_check)
            check_thread.daemon = True
            check_thread.start()

    # ~~ EventHandlerPlugin mixin (No changes needed) ~~
    def on_event(self, event, payload):
        if event == "PrintStarted":
            ...
            self.is_printing = False
            # Send a final "idle" status update to the UI
            self._plugin_manager.send_plugin_message(self._identifier, dict(status="idle"))
            
    # ~~ Core Logic (detection_loop is the same, perform_check is MODIFIED) ~~
    ...
    def get_update_information(self):
        return dict(
            failuredetector=dict(
                ...
                type="github_release",
                user="YourUsername",
                repo="OctoPrint-FailureDetector",
                current=self._plugin_version,
                pip="https://github.com/YourUsername/OctoPrint-FailureDetector/archive/{target_version}.zip"
            )
        )
